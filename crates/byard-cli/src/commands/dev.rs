...
impl App {
    ...
    /// time) with this thread's own ring, which is where `EncoderSubsystem`
    /// pushes GPU pass samples during `render_latest` — see
    /// `telemetry_overlay`'s module docs for why the two live on separate
    /// rings.
    fn print_telemetry_overlay(engine: &Engine, last_print: &mut std::time::Instant) {
        if last_print.elapsed() < std::time::Duration::from_secs(1) {
            return;
        }
        *last_print = std::time::Instant::now();
        let cpu = engine.latest_cpu_telemetry().unwrap_or_default();
        let gpu = byard_core::telemetry::drain_samples();
...
impl PlatformHost for App {
    ...
    fn on_redraw(&mut self) -> Result<(), ByardError> {
        if let Some(e) = self.engine.as_mut() {
            e.render_latest()?;
            App::print_telemetry_overlay(e, &mut self.last_telemetry_print);
        }
        Ok(())
    }
